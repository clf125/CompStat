...
$$U \leq \alpha' f(y)/p(y) \Leftrightarrow \log(U) \leq \log f(y) + \log \alpha' - \log p(y)$$
...
Profiling `log_rejection_sample_vec` (200,000 samples) with `profvis`/`Rprof` — the call is shown at the top of the screenshot:

![](profiling_before.png){width="80%"}

**6570 of 6665 ms of self time 98.5% is on one line** not the math itself, but calling a fresh R closure once per proposal via `sapply`.
...
So $\ell$ is **concave** — hence we can use it as our proposal for the piecewise log-affine envelope.
...
$$
z_j = \frac{b_{j+1} - b_j}{a_j - a_{j+1}}.
$$

With $z_0 = 0$ and $z_k = \infty$, these define the pieces. The lowest tangent gives $V(y) = a_j y + b_j$ on $[z_{j-1}, c_j]$, hence

$$
\alpha'q(y) \leq e^{V(y)}.
$$

Tangency means equality at the chosen points $\Rightarrow$ we can use $\alpha' = 1$ as this works already.
...
Since tangency gives equality at each $t_j$, $\alpha' = 1$ — no separate normalizing-constant step is needed.
...
A fixed, non-adaptive grid of $m$ tangent points, spaced over $[0, \hat{y} + 3\hat{\sigma}]$ (mode $\pm$ curvature-based spread). More points yields a higher acceptance rate, but each proposal also costs more to build and sample from.
...
Time (left) drops sharply from $m=2$ to $m=5$, and is then mostly flat — re-running the sweep does not reliably pick the same argmin. Acceptance rate (right) is monotone and stable, so we take the smallest $m$ crossing 99% (dashed line): $m =$ `r out$best$m` (red point).
