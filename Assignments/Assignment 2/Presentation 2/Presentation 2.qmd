...
- The posterior for a single coefficient in a Poisson regression model
...
#| fig-height: 4
#| fig-align: center
setwd("C:/Users/carla/Documents/Computational Statistics")
source("Assignment/Assignment 2/Target_setup.R")
A <- sum(dat$x * (dat$z - 1)); B <- sum(dat$x^2)
mu <- A / B; sigma <- sqrt(1 / B)
log_alpha_prime <- 100 - A^2/(2*B) - log(sigma*sqrt(2*pi))
...
|------------------------------------|------------------|
| `FUN` (inner function of `sapply`) | 80.4%            |
| `$` (data frame column access)     | 7.9%             |
| `lapply`                           | 6.0%             |
| `sum`                              | 3.6%             |
| `dnorm`, `rnorm`, `runif`, other   | 1.7%             |

**97.8% of runtime is inside `log_f_vec`** — not the math itself, but calling a fresh R function once per proposal via `sapply`.
...
| Function               | \% of total time |
|------------------------|------------------|
| `log_f_vec_faster`     | 97.8%            |
| `colSums`              | 94.9%            |
| `exp`                  | 15.2%            |
| `outer` / `tcrossprod` | 2.9%             |
...
out <- readRDS("Assignment/Assignment 2/Presentation 2/best_m.rds")
```
...
- Not R vs. C++: it's acceptance rate. The Gaussian envelope accepts only $\approx$ 13% of proposals, needing $\approx$ 7--8$\times$ as many (expensive) evaluations of the 100-term log-likelihood sum per accepted sample
- `log_affine_m5` (87.7% acceptance) and `log_affine_m20_best` (99.5%) are nearly identical here — the best $m$ buys a much tighter envelope almost for free
- `cpp_specialized_gaussian` and `r_vectorized_faster_gaussian` stay close together — a tight C++ loop doesn't fix a low acceptance rate
