...
\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{Presentation-2_files/figure-beamer/unnamed-chunk-1-1.pdf}}
\end{center}
...
\texttt{profvis}/\texttt{Rprof} --- the call is shown at the top of the
screenshot:

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{profiling_before.png}

\textbf{6570 of 6665 ms of self time 98.5\% is on one line} not the math
itself, but calling a fresh R closure once per proposal via
\texttt{sapply}.
\end{frame}
...
\NormalTok{  xy }\OtherTok{\textless{}{-}} \FunctionTok{outer}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{x, y)}
...
With \(z_0 = 0\) and \(z_k = \infty\), these define the pieces. The
lowest tangent gives \(V(y) = a_j y + b_j\) on \([z_{j-1}, c_j]\), hence
...
\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{Presentation-2_files/figure-beamer/unnamed-chunk-3-1.pdf}}
\end{center}
