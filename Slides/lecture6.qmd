...
## Solution: Step 1 — Estimate the Mean

The density is symmetric about zero, so $x f(x)$ is odd and
$\E X = \int_{-\pi}^{\pi} x f(x)\,\mathrm{d}x = 0$.
Use the proposal $g = \mathcal{N}(0,(\pi/2)^2)$ and ordinary IS:
$$
  \hat\mu_n = \frac{1}{n}\sum_{i=1}^n X_i w_i^*,
  \qquad w_i^* = \frac{f(X_i)}{g(X_i)}.
$$
The target density is normalized, so there is no need to divide by the sum
of weights. Draws outside $[-\pi,\pi]$ receive zero weight and remain in the
sample of size $n$.
...
vm_running_mean <- cumsum(vm_values) / seq_len(vm_N)
```

## Solution: Step 1 — Convergence

```{r}
#| label: vmises-convergence
#| echo: false
#| fig-width: 5
#| fig-height: 2.8
#| fig-cap: "Running ordinary IS estimate of the von Mises arithmetic mean."

plot(
  seq_len(vm_N), vm_running_mean,
  type = "l", col = "steelblue",
  xlab = "Number of samples", ylab = "Estimated mean"
)
abline(h = 0, col = "darkorange", lty = 2)
```

The running estimate approaches zero by the law of large numbers; its path
need not be monotone.

## Solution: Step 2 — Standard Error

Set $Y_i = X_i w_i^*$. The estimate is the sample mean of the $Y_i$, so
$$
...
$$
Use the standard deviation of the weighted contributions, not the sampled
$X_i$ or the weights alone.

```{r}
#| label: vmises-standard-error
...
## Solution: Step 3 — Weight Diagnostics
...
```{r}
#| label: vmises-weight-diagnostics
...
## Solution: Step 3 — Compare Proposals

Try normal proposals with standard deviations $1$ and $1.3$, which concentrate
more draws near the target's mode, and $\mathrm{Uniform}(-\pi,\pi)$, which
covers exactly its support. All four proposals are positive throughout the
target's support.

```{r}
#| label: vmises-proposal-comparison
...
knitr::kable(vm_comparison, digits = c(0, 5, 5, 0, 3), row.names = FALSE)
```

## Solution: Step 3 — Interpretation

At the same sample size, the smallest estimated standard error in this run is
obtained with **`r vm_comparison$proposal[which.min(vm_comparison$SE)]`**.
Compare standard errors to judge precision for this particular mean; proximity
of one estimate to zero can be luck.

Higher ESS and lower CV indicate more even weights, but do not necessarily
give a smaller standard error: that also depends on the integrand $h(x)=x$.
For ordinary IS the ideal proposal is proportional to $|x|f(x)$, rather than
just $f(x)$. Thus concentrating too tightly around zero can miss useful tail
contributions. These diagnostics are empirical, so repeat the comparison with
other seeds or larger samples before drawing strong conclusions.
